...
describe('Auth API', () => {
  ...
  test('POST /api/register creates a user', async () => {
    const res = await request(app)
      .post('/api/register')
      .send({ username: 'alice', password: 'pass123', field: 'Biology' });
  ...
  test('POST /api/register with duplicate username returns 409', async () => {
    await request(app).post('/api/register').send({ username: 'bob', password: 'pass123' });
    const res = await request(app).post('/api/register').send({ username: 'bob', password: 'other' });
  ...
  test('POST /api/login with correct credentials returns user', async () => {
    const res = await request(app).post('/api/login').send({ username: 'alice', password: 'pass123' });
    expect(res.status).toBe(200);
    expect(res.body.username).toBe('alice');
  });
});
...
describe('Rooms API', () => {
  ...
  test('POST /api/rooms creates a room when authenticated', async () => {
    ...
    await agent.post('/api/register').send({ username: 'carol', password: 'pass123' });
    const res = await agent.post('/api/rooms').send({ name: 'Quantum Physics', description: 'Q stuff' });
    expect(res.status).toBe(201);
  ...
  test('POST /api/rooms without name returns 400', async () => {
    const agent = request.agent(app);
    await agent.post('/api/register').send({ username: 'dave', password: 'pass123' });
    const res = await agent.post('/api/rooms').send({ description: 'no name' });
    expect(res.status).toBe(400);
  });
});
...
describe('Ideas API', () => {
  ...
  test('POST /api/ideas requires auth', async () => {
    const res = await request(app).post('/api/ideas').send({ title: 'Test', body: 'Body' });
    expect(res.status).toBe(401);
  });

  test('POST /api/ideas creates an idea when authenticated', async () => {
    const agent = request.agent(app);
    await agent.post('/api/register').send({ username: 'eve', password: 'pass123' });
    const res = await agent.post('/api/ideas').send({ title: 'ML in Ecology', body: 'Using ML to track species', tags: ['ML', 'ecology'] });
    expect(res.status).toBe(201);
  ...
  test('POST /api/ideas/:id/like increments likes', async () => {
    const agent = request.agent(app);
    await agent.post('/api/register').send({ username: 'frank', password: 'pass123' });
    const ideaRes = await agent.post('/api/ideas').send({ title: 'Idea X', body: 'Body X' });
    const id = ideaRes.body.id;
    const likeRes = await agent.post(`/api/ideas/${id}/like`);
  ...
  test('POST /api/ideas/:id/like twice returns 409', async () => {
    const agent = request.agent(app);
    await agent.post('/api/register').send({ username: 'grace', password: 'pass123' });
    const ideaRes = await agent.post('/api/ideas').send({ title: 'Idea Y', body: 'Body Y' });
    const id = ideaRes.body.id;
    await agent.post(`/api/ideas/${id}/like`);
    const res = await agent.post(`/api/ideas/${id}/like`);
    expect(res.status).toBe(409);
  });
});
